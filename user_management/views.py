...
from .models import (
    ContactMessage, TeamMember, Post, NewsItem, CustomUser, Member, Progress
)
from .forms import (
...
def update_progress(request):
    if request.method == 'POST':
        ...
        # استرجاع أو إنشاء الكائن لتحديث القيم
        progress = Progress.objects.first()
        if not progress:
            progress = Progress.objects.create(partner=partner, employee=employee, client=client, job=job)
        else:
            progress.partner = partner
        ...
        return redirect('index')  # التأكد من أن "index" هو اسم المسار الصحيح
